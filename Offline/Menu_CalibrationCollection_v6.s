...
TagGroup TomoDialog()
{
	...
	TagGroup Loadbutton = DLGCreatePushButton("(Re)Load Python", "Load")
	...
	TagGroup Load_box = DLGCreateBox("Model Settings", Load_items) 
	...
	TagGroup Tilt_box = DLGCreateBox("Tilt Settings", tilt_items)
	...
	TagGroup DeltaTiltField_Label = DLGCreateLabel("Step")
	TagGroup delta_combined = DLGGroupitems(DeltaTiltField_Label,DeltaTiltField).dlgtablelayout(1,2,1)
	
	TagGroup Tilt_Block = DLGGroupItems(min_combined,delta_combined,max_combined).dlgtablelayout(3,1,0)
	...
	//X_Next Row
	TagGroup X_NextLabel = DLGCreateLabel("XCORR Next: ")
	TagGroup X_NextX = DLGCreateRealField(0,9,5).dlgidentifier("X_nextX")
	TagGroup X_NextY = DLGCreateRealField(0,9,5).dlgidentifier("X_nextY")
	TagGroup X_NextZ = DLGCreateLabel("")
	TagGroup X_NextDF = DLGCreateLabel("")
	TagGroup X_NextTilt = DLGCreateLabel("")
	...
	DLGexternalpadding(X_nextLabel,0,2)
	
	TagGroup LabelVals = DLGgroupitems(pastLabel,currLabel,nextLabel,X_nextLabel).DLGTableLayout(1,4,0)
	TagGroup Label_combined = DLGgroupitems(EmptyLabel,LabelVals).DLGTablelayout(1,2,0)

	TagGroup XVals = DLGgroupitems(pastX,currX,nextX,X_nextX).DLGTableLayout(1,4,1)
	TagGroup X_combined = DLGgroupitems(XLabel,XVals).DLGTablelayout(1,2,0)
	
	TagGroup YVals = DLGgroupitems(pastY,currY,nextY,X_nextY).DLGTableLayout(1,4,1)
	TagGroup Y_combined = DLGgroupitems(YLabel,YVals).DLGTablelayout(1,2,0)
	
	TagGroup ZVals = DLGgroupitems(pastZ,currZ,nextZ,X_nextZ).DLGTableLayout(1,4,1)
	TagGroup Z_combined = DLGgroupitems(ZLabel,ZVals).DLGTablelayout(1,2,0)
	
	TagGroup DFVals = DLGgroupitems(pastDF,currDF,nextDF,X_nextDF).DLGTableLayout(1,4,1)
	TagGroup DF_combined = DLGgroupitems(DFLabel,DFVals).DLGTablelayout(1,2,0)
	
	TagGroup TiltVals = DLGgroupitems(pastTilt,currTilt,nextTilt,X_nextTilt).DLGTableLayout(1,4,1)
	...
	TagGroup TiltButtons_combined = DLGGroupitems(TiltMinus_button,TiltPlus_button).DLGtableLayout(2,1,1)
	...
	TagGroup XCorr_combined = DLGGroupitems(CalcXCorr_button,ShiftXCorr_button).dlgtablelayout(1,2,1)
...
class MainMenu : uiframe
{
	...
	//MODIFIES: stageTilt
	//EFFECTS : Takes Microsocpe given alpha, adds step to it for a shift!
	void tilt_plus(object self)
	{
		result("Beginning tilt...\n")
		RefreshFunction_past(self)
		number tilt = 0 //EMGetStageAlpha()
	...
	void calc_x_corr(object self){
		...
		number magIndex = 0//EMGetMagIndex()
		//EMSetMagIndex(magIndex-1)
		number sx, sy
		number imagex,imagey
		...
		//EMGetStagePositions(3,imagex,imagey,0,0,0)
		X_nextX = sx+imagex
		X_nextY = sy+imagey
		DLGvalue(self.lookupelement("X_nextX"),X_nextX)
		DLGvalue(self.lookupelement("X_nextY"),X_nextY)
		//EMSetMagIndex(magIndex)
	...
	void shift_x_corr(object self){
		...
		//EMGetStagePositions(3,currX,currY,0,0,0)
		changeX = abs(newX-currX)
		changeY = abs(newY-currY)
